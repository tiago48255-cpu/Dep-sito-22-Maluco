...
import { ArrowLeft, Star, Clock, Thermometer, Sparkles, Zap, Beer, Plus, Heart } from "lucide-react";
...
export default async function ProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="text-white min-h-screen pb-32 md:pb-8 bg-black">
      <header className="page-header">
        <Link
          href="/"
          className="p-2 hover:bg-neutral-900 rounded-full transition-colors text-neutral-400 hover:text-white shrink-0"
        >
          <ArrowLeft className="w-5 h-5" />
        </Link>
        <div className="flex flex-col items-center flex-1 text-center">
          <span className="text-[9px] text-neutral-400 font-bold uppercase tracking-widest leading-none">Produto</span>
          <h1 className="text-xs font-bold text-white mt-0.5 capitalize">{product.category}</h1>
        </div>
        <button className="p-2 text-neutral-400 hover:text-brand-secondary transition-colors shrink-0">
      ...
      <div className="md:flex md:gap-8 content-container md:pt-8">
        {/* Product Image */}
        <section className="relative w-full md:w-1/2 md:max-w-[560px] aspect-[4/5] md:aspect-square overflow-hidden bg-[#0A0A0A] select-none md:rounded-2xl md:shrink-0">
          <div className="absolute inset-x-0 bottom-0 h-2/5 bg-gradient-to-t from-neutral-950 via-neutral-950/60 to-transparent z-10 pointer-events-none md:hidden" />
          {product.image_url ? (
            <Image
        ...
        <section className="flex-1 flex flex-col gap-5 -mt-6 md:mt-0 relative z-30 text-left px-5 md:px-0">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            {product.description && (
              <p className="text-xs md:text-sm text-neutral-400 leading-relaxed font-medium">{product.description}</p>
            )}
            {product.stock_qty === 0 && (
